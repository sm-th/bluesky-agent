...
def build_research_prompt(turn: Turn, operator_handle: str | None = None) -> str:
    """Build a bounded prompt that treats the Bluesky post as untrusted input."""
...
3. Produce exactly one new Research Page for this turn at {page_path}; do not create any other file under wiki/research/. Follow the research-page schema in AGENTS.md exactly. Its required frontmatter values are `rkey: {turn.rkey}`, `date: {turn.created_at}`, and `turn_url: {turn_url}`. Its `brief` value must exactly equal the terminal Brief Reply you emit. The site derives its stable deployed turn marker from the rkey.
4. The page's Answer is the Expanded Answer: it must directly address the substance of the Research Turn, stand alone, and remain at most {RESEARCH_PAGE_MAX_WORDS} words so the entire page is readable in one 1200x1600 screenshot. Target 140-180 words. Use short paragraphs, at most two compact sections, and dense useful prose; omit background and process narrative.
